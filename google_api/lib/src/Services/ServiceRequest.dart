...
import 'package:google_api/src/Network/NetworkListener.dart';
...
class ServiceRequest {
  ...
  Future<T> getRequest<T, K>(URLRequest urlRequest) async {
    try {
      Response response = await _dio.get(urlRequest.url, queryParameters: urlRequest.parameters);
      return ProcessJson.fromJson<T,K>(response.data);
    } on DioError catch (e) {
      /// The request was made and the server responded with a status code
      /// that falls out of the range of 2xx and is also not 304.
      if (e.response != null) {
        throw Future.error('STATUS: ${e.response?.statusCode} DATA: ${e.response?.data} HEADERS: ${e.response?.headers} HEADERS: ${e.response?.headers}');
      } else {
        /// Error due to setting up or sending the request
        throw Future.error('${ServiceError.unknownStatusCode.description} ${e.error}' );
      }
    }
  }
}